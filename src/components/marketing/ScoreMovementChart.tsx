    ...
    <div className="space-y-5">
      ...
      >
        <div
          ...
          style={{
            backgroundImage:
              "repeating-linear-gradient(180deg, transparent 0px, transparent 2px, color-mix(in srgb, var(--ink-950) 24%, transparent) 3px)",
          }}
        />
        <div className="mb-4 flex items-center justify-between relative z-10">
          <div>
            <p className="text-[10px] uppercase tracking-[0.15em] text-neon-cyan" style={{ fontFamily: "var(--font-cosmic-display)" }}>
        ...
        <div className="relative z-10" style={{ height: 160 }}>
          <svg viewBox={`0 0 ${WEEKS.length * 60} 160`} className="w-full" style={{ overflow: 'visible' }}>
            {[50, 60, 70].map((val) => {
              ...
              return (
                <g key={val}>
                  <line x1={0} x2={WEEKS.length * 60} y1={y} y2={y} stroke="var(--constellation-line)" />
                  <text x={-4} y={y + 3} fill="var(--neon-cyan)" fontSize={10} textAnchor="end" opacity="0.72" style={{ fontFamily: "var(--font-cosmic-display)" }}>
                    {val}
                  </text>
                </g>
              );
              ...
              return (
                <g key={i}>
                  ...
                  <circle
                    ...
                    stroke="var(--neon-cyan)"
                    strokeWidth={2}
                    style={{ filter: "drop-shadow(0 0 7px var(--neon-cyan))" }}
                  />
                  <text x={p.x} y={155} fill="var(--neon-cyan)" opacity="0.8" fontSize={10} textAnchor="middle" style={{ fontFamily: "var(--font-cosmic-display)" }}>
                    W{w.week}
                  </text>
                </g>
              );
