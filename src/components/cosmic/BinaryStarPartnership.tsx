    ...
    <div className="glass-card p-5">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" aria-label={`Binary star partnership — ${isAligned ? 'aligned' : 'drifting'}`}>
        ...
        <g>
          ...
          <circle cx={half / 2} cy={H / 2} r={20} fill="var(--text-body)" opacity={0.08} filter="url(#bsp-glow)" />
          ...
          <text x={half / 2 - 55} y={H / 2 + 28} textAnchor="middle" fill="var(--gold-primary)" fontSize="8" opacity={0.5}>{starA}</text>
          <text x={half / 2 + 55} y={H / 2 + 28} textAnchor="middle" fill="var(--ray-possibility)" fontSize="8" opacity={0.5}>{starB}</text>
          <text x={half / 2} y={H - 16} textAnchor="middle" fill="var(--gold-primary)" fontSize="10" fontWeight="600" opacity={0.5}>ALIGNED</text>
        </g>
        ...
        <g transform={`translate(${half + 8}, 0)`}>
          ...
          <ellipse cx={half / 2 - 30} cy={H / 2} rx={55} ry={40} fill="none" stroke="var(--text-body)" strokeWidth={0.5} strokeOpacity={0.1} />
          <ellipse cx={half / 2 + 40} cy={H / 2} rx={70} ry={50} fill="none" stroke="var(--text-body)" strokeWidth={0.5} strokeOpacity={0.08} transform={`rotate(15 ${half / 2 + 40} ${H / 2})`} />

          {/* Star A — same position, same brightness */}
          <circle cx={half / 2 - 70} cy={H / 2 - 10} r={14} fill="var(--gold-primary)" opacity={0.65} />

          {/* Star B — dimmer, farther */}
          <circle cx={half / 2 + 90} cy={H / 2 + 20} r={12} fill="var(--ray-possibility)" opacity={0.45} />

          {/* Empty purple gap where combined glow used to be */}
          <circle cx={half / 2} cy={H / 2} r={15} fill="none" stroke="var(--text-body)" strokeWidth={0.3} strokeOpacity={0.05} strokeDasharray="2 4" />

          <text x={half / 2 - 70} y={H / 2 + 28} textAnchor="middle" fill="var(--gold-primary)" fontSize="8" opacity={0.4}>{starA}</text>
          <text x={half / 2 + 90} y={H / 2 + 42} textAnchor="middle" fill="var(--ray-possibility)" fontSize="8" opacity={0.3}>{starB}</text>
          <text x={half / 2} y={H - 16} textAnchor="middle" fill="var(--text-muted)" fontSize="10" fontWeight="600" opacity={0.35}>DRIFTING</text>
        </g>
      </svg>
      ...
      <div className="mt-3 flex items-center gap-3">
        <div style={{ flex: 1, height: 4, borderRadius: 2, background: 'var(--surface-glass)', overflow: 'hidden' }}>
          <motion.div
            style={{ height: '100%', borderRadius: 2, background: isAligned ? 'var(--gold-primary)' : 'var(--neon-violet)' }}
            initial={false}
