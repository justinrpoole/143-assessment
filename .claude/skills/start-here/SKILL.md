...
```
  ...
  PRODUCT (build + optimize apps)
  Skill                    Purpose                         Status
  ─────────────────────────────────────────────────────────────
  /app-developer           Audit, upgrade, build apps      custom

  INTELLIGENCE (assessment + behavioral science)
  Skill                    Purpose                         Status
  ─────────────────────────────────────────────────────────────
  /assessment-engine       9-Ray scoring, exec reports     custom
  /assessment-brain        Participant + exec reports      custom

  RESEARCH (evidence + competitive)
  Skill                    Purpose                         Status
  ─────────────────────────────────────────────────────────────
  /knowledge-vault          Evidence vault + citations       custom
  /competitive-social-research Competitor social intel     custom

  COACHING (coach anyone through anything)
  Skill                    Purpose                         Status
  ─────────────────────────────────────────────────────────────
  /coaching-engine         3-tier coaching, 9 tools        custom

  QA + HARDENING (audit + protect)
  Skill                    Purpose                         Status
  ─────────────────────────────────────────────────────────────
  /bralph                  Baseline, drift, QA, harden     custom

  DISTRIBUTION (spread everything)
  Skill                    Purpose                         Status
  ─────────────────────────────────────────────────────────────
  /content-atomizer        Repurpose across 8 platforms    v2.0

  INTEGRATION (ship visuals)
  Skill                    Purpose                         Status
  ─────────────────────────────────────────────────────────────
  /visual-integrator       Integrate visuals into apps     custom
...
```
  Layer 0: SYSTEM (runs before everything)
  ├── /brand-switch              active-brand.md (brand context gate)
  └── /preflight                 stack.md (tool readiness)

  Layer 1: FOUNDATION (run first — builds brand memory)
  ...
  Layer 4: DISTRIBUTION (needs execution assets)
  ├── /content-atomizer          social, threads, shorts
  └── /creative ad-mode          paid ad variants

  Layer 5: PRODUCT + INTEGRATION (needs foundation + stack)
  ├── /app-developer             audit reports, code, UX architecture
  └── /visual-integrator         production visuals, optimized assets

  Layer 6: INTELLIGENCE + RESEARCH (needs foundation + 143 source library)
  ├── /assessment-engine         scored assessments, exec reports
  ├── /assessment-brain          participant + executive reports
  ├── /knowledge-vault            evidence vault + citations
  └── /competitive-social-research competitor research

  Layer 7: COACHING (needs foundation + 143 source library)
  └── /coaching-engine           coaching sessions, reps, tools

  Layer 8: QA + HARDENING (needs stack + running app)
  └── /bralph                    baselines, drift reports, QA issues
```
...
and explain why. Layers 5-8 can be invoked at any time independently —
they describe specialized capabilities, not enforced order.

**Brand-switch is NOT optional.** It runs before any other skill on
every session start. If `./brand/active-brand.md` does not exist or
was not written today, run /brand-switch first to select which business
the user is working on. Loading the wrong voice contaminates everything
downstream.
...
the ./brand/ directory. But FIRST, run the brand-switch gate.

### Check 0: Brand-Switch Gate (MANDATORY)

Before anything else, check `./brand/active-brand.md`:
- **File does not exist** → Run /brand-switch. Ask which business.
- **File exists but was NOT written today** → Run /brand-switch.
  Show current brand, confirm or switch.
- **File exists and was written today** → Show one-line
  confirmation: "Working as {brand name}." Continue.

This is NOT optional. The wrong brand voice contaminates everything
downstream. Four businesses share one system:
- Justin Ray / 143 Leadership
- JRW Design Co / Handmade Signs
- Ohio Made
- Consulting / PM & Leadership

After brand-switch completes, load brand files from the correct
...
│
├─ Contains "switch brand" / "wrong voice" / "which business"
│  / "change brand"
│  └─ /brand-switch
│     Context: brands.md, active-brand.md
│
├─ Contains "app" / "UX" / "retention" / "onboarding"
│  / "build a coaching app" / "audit my app" / "data model"
│  / "feedback loops" / "app architecture"
│  └─ /app-developer
│     Context: voice profile, positioning, audience, stack
│
├─ Contains "score assessment" / "survey results" / "Ray"
│  / "light signature" / "eclipse" / "burnout risk"
│  / "psychometric" / "construct" / "validity"
│  / "benchmark" / "executive report" / "cohort"
│  / "JW framework" / "143 scoring" / "assessment"
│  └─ /assessment-engine
│     Context: voice profile, positioning, audience, stack,
│     143 Leadership source library
│
├─ Contains "coach me" / "I'm stuck" / "I'm anxious"
│  / "goal" / "habit" / "confidence" / "fear"
│  / "leadership coaching" / "executive coaching"
│  / "Be The Light" / "my assessment" / "give me a rep"
│  / "what tool" / "Eclipse"
│  └─ /coaching-engine
│     Context: voice profile, positioning, audience,
│     143 Leadership source library
│
├─ Contains "run BRALPH" / "baseline" / "drift check"
│  / "QA audit" / "route check" / "link check"
│  / "layout audit" / "nav audit" / "smoke test"
│  / "tone lint" / "content audit" / "harden"
│  / "regression check" / "full audit" / "run qa"
│  └─ /bralph
│     Context: voice profile, positioning, audience, stack,
│     previous baselines from ./qa/
│
...
### Workflow 8: BUILD APP

**Trigger:** "build my app", "audit my app", "app development",
"coaching app", "improve my UX", "fix retention"

**Chain:**
```
  Step 1: /app-developer (audit mode)
  → Competitive teardown + gap analysis
  → Outputs: audit-report.md, architecture.md

  Step 2: /app-developer (targeted upgrades)
  → Research-backed, systems-level improvements
  → Runs audit-verify-auto-accept loops

  Step 3: /direct-response-copy
  → Landing page for the app
  → Uses voice profile + positioning from brand memory

  Step 4: /content-atomizer
  → Social promotion for the app launch
```

**Context passing:**
- app-developer gets: voice profile, positioning, audience, stack
- direct-response-copy gets: voice profile, positioning, app
  description + key features (from step 2)
- content-atomizer gets: voice profile, app name + key benefit,
  landing page URL placeholder
...
### Workflow 9: ASSESSMENT INTELLIGENCE

**Trigger:** "score my assessment", "assessment results",
"run the assessment", "executive report", "benchmark results"

**Chain:**
```
  Step 1: /assessment-engine (score)
  → Process survey data, run JW Framework scoring
  → Output: scored assessment with 9-Ray profile

  Step 2: /assessment-engine (interpret)
  → Light Signature archetype, Eclipse patterns,
    burnout risk, capacity gaps

  Step 3: /assessment-engine (executive report)
  → Executive-grade intelligence report with
    coaching recommendations

  Step 4: /email-sequences (result delivery)
  → Automated result delivery emails with
    personalized coaching next steps
```

**Context passing:**
- assessment-engine gets: voice profile, positioning, audience,
  stack, 143 Leadership source library
- email-sequences gets: voice profile, assessment results summary,
  coaching recommendations (from step 3)

**Estimated time:** 45-60 minutes total

---

### Workflow 10: ASSESSMENT LAUNCH

**Trigger:** "launch my assessment", "sell the assessment",
"assessment marketing", "promote 143"

**Chain:**
```
  Step 1: /assessment-engine (research)
  → Competitive intelligence, market positioning

  Step 2: /positioning-angles
  → Assessment-specific positioning angles

  Step 3: /direct-response-copy
  → Assessment landing page copy

  Step 4: /email-sequences
  → Assessment promotion + result delivery sequences

  Step 5: /content-atomizer
  → Social promotion across all platforms
```

**Context passing:**
- assessment-engine gets: voice profile, positioning, audience, stack
- positioning-angles gets: assessment research (from step 1),
  audience, competitors
- direct-response-copy gets: voice profile, chosen angle (from
  step 2), assessment details
- email-sequences gets: voice profile, angle, assessment details,
  landing page headline (from step 3)
- content-atomizer gets: voice profile, assessment name + key
  benefit, landing page URL placeholder

**Estimated time:** 90-120 minutes total

---

### Workflow 11: FULL QA AUDIT

**Trigger:** "run BRALPH", "full audit", "QA my app",
"check for drift", "regression check", "harden my app"

**Chain:**
```
  Step 1: /bralph (baseline + audit)
  → Build master baseline, run full audit suite
  → Route check, link check, content audit, tone lint,
    layout audit, a11y check

  Step 2: /bralph (patch + retest)
  → Fix identified issues, re-run failing checks
  → Loop until all checks pass

  Step 3: /app-developer (architectural fixes)
  → Address any structural issues found by BRALPH
  → that require code-level changes

  Step 4: /bralph (harden)
  → Lock standards, write Playwright tests,
    encode assumptions, final regression pass
```

**Context passing:**
- bralph gets: voice profile, positioning, audience, stack,
  previous baselines from ./qa/
- app-developer gets: QA issues table (from step 1-2), stack,
  specific architectural problems identified
- bralph (harden) gets: updated codebase state (from step 3),
  previous audit results for regression comparison

**Estimated time:** 60-90 minutes total

---

### Workflow 12: SHIP WITH CONFIDENCE

**Trigger:** "ship it", "launch with QA", "build and test",
"deploy with confidence"

**Chain:**
```
  Step 1: /app-developer (build)
  → Implement the feature or upgrade

  Step 2: /bralph (audit)
  → Full baseline + drift + QA audit post-build

  Step 3: /bralph (patch)
  → Fix any issues found, re-run until green

  Step 4: /content-atomizer (launch)
  → Social promotion for the shipped feature
```
...
- bralph gets: voice profile, positioning, audience, stack,
  previous baselines
- content-atomizer gets: voice profile, feature name + key benefit,
  launch angle
...
```
  ...
  Build or audit my app         Workflow 8 (BUILD APP)
  Score assessments             Workflow 9 (ASSESSMENT INTELLIGENCE)
  Launch my assessment          Workflow 10 (ASSESSMENT LAUNCH)
  QA my app                     Workflow 11 (FULL QA AUDIT)
  Ship with confidence          Workflow 12 (SHIP WITH CONFIDENCE)
  Get coached                   /coaching-engine
...
```
  ...
  App audit or architecture     /app-developer
  Assessment scoring            /assessment-engine
  QA baselines or drift checks  /bralph
  Coaching sessions             /coaching-engine
  Brand context (wrong voice)   /brand-switch
...
```
  ...
  App audit                     /app-developer (~30 min)
  Assessment scoring            /assessment-engine (~20 min)
  Coaching session              /coaching-engine (~15 min)
  QA smoke test                 /bralph (~20 min)
```
...
```
  ...
  /brand-switch
  ├── PASS: brands.md (brand registry)
  ├── PASS: active-brand.md (current session state)
  └── WITHHOLD: everything else — this skill only selects
      the brand and loads the correct profile directory

  /app-developer
  ├── PASS: voice profile (full — shapes app copy/UX tone)
  ├── PASS: positioning angle (for product direction)
  ├── PASS: audience profile (pain points, desired outcome)
  ├── PASS: stack.md (tech stack, connected tools)
  └── WITHHOLD: keyword plan, email data, creative kit,
      competitor analysis, campaign history, newsletter

  /assessment-engine
  ├── PASS: voice profile (full — for report tone)
  ├── PASS: positioning angle (for assessment framing)
  ├── PASS: audience profile (who takes the assessment)
  ├── PASS: stack.md (for implementation context)
  ├── PASS: 143 Leadership source library (JW Framework,
      researcher pillars, scoring methodology)
  └── WITHHOLD: keyword plan, email data, creative kit,
      campaign history, newsletter, competitor analysis

  /coaching-engine
  ├── PASS: voice profile (full — Justin Ray voice is
      non-negotiable for coaching sessions)
  ├── PASS: positioning angle (for coaching framing)
  ├── PASS: audience profile (who is being coached)
  ├── PASS: 143 Leadership source library (coaching tools,
      rep library, assessment data if available)
  └── WITHHOLD: keyword plan, email data, creative kit,
      stack.md, campaign history, competitor analysis

  /bralph
  ├── PASS: voice profile (for tone lint audits)
  ├── PASS: positioning angle (for content audit alignment)
  ├── PASS: audience profile (for UX/a11y context)
  ├── PASS: stack.md (tech stack, URLs, deployment info)
  ├── PASS: previous baselines from ./qa/ (for drift detection)
  └── WITHHOLD: keyword plan, email data, creative kit,
      campaign history, newsletter, competitor analysis
```
...
```
  STEP 0: BRAND GATE (mandatory, runs first)
  ✓ Check ./brand/active-brand.md
  ✓ If missing or stale → run /brand-switch
  ✓ If fresh → confirm brand in one line
  ✓ Load brand files from correct profile directory
  ...
  ✓ Read ./qa/ directory (any baselines or audit history?)
  ✓ Read ./assessments/ directory (any scored assessments?)
  ...
  ✓ Identify gaps (including app, assessment, QA gaps)
...
### Full Skill Invocation Map

When dispatching any skill, use this reference for the correct
SKILL.md location and context to pass:

```
  Skill                 SKILL.md location
  ──────────────────────────────────────────────────
  /brand-switch         ~/.claude/skills/brand-switch/SKILL.md
  /brand-voice          ~/.claude/skills/brand-voice/SKILL.md
  /positioning-angles   ~/.claude/skills/positioning-angles/SKILL.md
  /keyword-research     ~/.claude/skills/keyword-research/SKILL.md
  /lead-magnet          ~/.claude/skills/lead-magnet/SKILL.md
  /direct-response-copy ~/.claude/skills/direct-response-copy/SKILL.md
  /seo-content          ~/.claude/skills/seo-content/SKILL.md
  /email-sequences      ~/.claude/skills/email-sequences/SKILL.md
  /newsletter           ~/.claude/skills/newsletter/SKILL.md
  /content-atomizer     ~/.claude/skills/content-atomizer/SKILL.md
  /creative             ~/.claude/skills/creative/SKILL.md
  /app-developer        ~/.claude/skills/app-developer/SKILL.md
  /assessment-engine    ~/.claude/skills/assessment-engine/SKILL.md
  /coaching-engine      ~/.claude/skills/coaching-engine/SKILL.md
  /bralph               ~/.claude/skills/bralph/SKILL.md
```
...
passes selective context, and tracks everything. 16 skills, 12
workflows, 4 brands, one router.*
