...
async function run() {
  ...
  const apostropheUnderscoreDotPlusDoubleTokenizedInsertionRes = await post({ name: 'QA Contract', email: `'${Date.now()}_.+user${Date.now()}${Date.now()}@example.com`, tag: 'qa-contract' });
  if (apostropheUnderscoreDotPlusDoubleTokenizedInsertionRes.status !== 400 || apostropheUnderscoreDotPlusDoubleTokenizedInsertionRes.json?.error !== 'invalid_email') {
    throw new Error(`leading apostrophe-underscore-dot-plus double tokenized insertion contract failed: status=${apostropheUnderscoreDotPlusDoubleTokenizedInsertionRes.status} body=${JSON.stringify(apostropheUnderscoreDotPlusDoubleTokenizedInsertionRes.json)}`);
  }
  console.log('ok:email-capture:leading-apostrophe-underscore-dot-plus-double-tokenized-insertion-invalid');
  ...
  const apostropheShiftedDotUnderscorePlusTripleTokenizedInsertionRes = await post({ name: 'QA Contract', email: `'${Date.now()}._+user${Date.now()}${Date.now()}${Date.now()}@example.com`, tag: 'qa-contract' });
  if (apostropheShiftedDotUnderscorePlusTripleTokenizedInsertionRes.status !== 400 || apostropheShiftedDotUnderscorePlusTripleTokenizedInsertionRes.json?.error !== 'invalid_email') {
    throw new Error(`leading apostrophe-shifted-dot-underscore-plus triple tokenized insertion contract failed: status=${apostropheShiftedDotUnderscorePlusTripleTokenizedInsertionRes.status} body=${JSON.stringify(apostropheShiftedDotUnderscorePlusTripleTokenizedInsertionRes.json)}`);
  }
  console.log('ok:email-capture:leading-apostrophe-shifted-dot-underscore-plus-triple-tokenized-insertion-invalid');
  ...
  const apostropheShiftedPlusUnderscoreDotDoubleTokenizedInsertionRes = await post({ name: 'QA Contract', email: `'${Date.now()}+_.user${Date.now()}${Date.now()}@example.com`, tag: 'qa-contract' });
  if (apostropheShiftedPlusUnderscoreDotDoubleTokenizedInsertionRes.status !== 400 || apostropheShiftedPlusUnderscoreDotDoubleTokenizedInsertionRes.json?.error !== 'invalid_email') {
    throw new Error(`leading apostrophe-shifted-plus-underscore-dot double tokenized insertion contract failed: status=${apostropheShiftedPlusUnderscoreDotDoubleTokenizedInsertionRes.status} body=${JSON.stringify(apostropheShiftedPlusUnderscoreDotDoubleTokenizedInsertionRes.json)}`);
  }
  console.log('ok:email-capture:leading-apostrophe-shifted-plus-underscore-dot-double-tokenized-insertion-invalid');
  ...
  const apostropheShiftedPlusDotUnderscoreDoubleTokenizedInsertionRes = await post({ name: 'QA Contract', email: `'${Date.now()}+._user${Date.now()}${Date.now()}@example.com`, tag: 'qa-contract' });
  if (apostropheShiftedPlusDotUnderscoreDoubleTokenizedInsertionRes.status !== 400 || apostropheShiftedPlusDotUnderscoreDoubleTokenizedInsertionRes.json?.error !== 'invalid_email') {
    throw new Error(`leading apostrophe-shifted-plus-dot-underscore double tokenized insertion contract failed: status=${apostropheShiftedPlusDotUnderscoreDoubleTokenizedInsertionRes.status} body=${JSON.stringify(apostropheShiftedPlusDotUnderscoreDoubleTokenizedInsertionRes.json)}`);
  }
  console.log('ok:email-capture:leading-apostrophe-shifted-plus-dot-underscore-double-tokenized-insertion-invalid');
